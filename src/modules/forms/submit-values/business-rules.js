...
//
// Kept in code, not Designer markup: same-named hidden <input>s gated by
// data-form-show-if cannot express "otherwise", so overlapping rows would let
// document order pick the winner. The cost is that these strings are not
 ...
 * when condition-hidden, so a stale sub-type is never sent.
 */
...
/**
 * appointment: where the meeting happens. Anything that is not a home visit is
 * in an office.
 */
 ...
 *
 * A home visit booked as a drop-off is 15.
 *
 * No default: an unrecognised sub_type sends no key, so a Designer copy edit
 * cannot silently book an hour.
 */
